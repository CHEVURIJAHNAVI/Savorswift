...
@login_required
def display_item_reviews(request):
    restaurant_user = request.user
    feedbacks = Feedback.objects.filter(item__restaurant_user=restaurant_user).order_by('-feedback_date')
    return render(request, 'restaurantapp/item_reviews.html', {'feedbacks': feedbacks})

from customerapp.models import Order
@login_required
def display_restaurant_orders(request):
    restaurant_user = request.user
    # Filter orders where the item belongs to the restaurant owner
    orders = Order.objects.filter(item__restaurant_user=restaurant_user).order_by('-order_date')
